...
def sendSortTransaction(address):
    ...
    tx_hash = sort_contract.functions.sort(5).transact({'txType':"0x2", 'from':w3.eth.accounts[0], 'gas':2000000})
    return tx_hash
...
def sendMatrixTransaction(address):
    ...
    address=address,
    abi=contract_interface['abi'])
    tx_hash = sort_contract.functions.multiply().transact({'txType':"0x2", 'from':w3.eth.accounts[0], 'gas':2000000})
    return tx_hash
...
def sendEmptyLoopTransaction(address):
    ...
    address=address,
    abi=contract_interface['abi'])
    tx_hash = sort_contract.functions.runLoop().transact({'txType':"0x2", 'from':w3.eth.accounts[0], 'gas':2000000})
    return tx_hash
